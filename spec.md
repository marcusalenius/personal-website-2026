...
| Icons | Phosphor Icons (React), but **inlined as cropped-viewBox wrappers** in `components/icons.tsx` rather than used directly — Phosphor's 256-unit box pads each glyph differently, shrinking visible ink and adding stray space next to labels; cropping the viewBox to the path bounds makes `size` map 1:1 to visible ink. `PaperPlaneTilt`, `GithubLogo`, `LinkedinLogo`, `ArrowLeft` (all regular weight); `ArrowUpRight` (**bold** weight, inlined in MDLink — heavier strokes hold up better at the size used for inline link glyphs) |
...
- **Footer**: 1px divider in `--color-line-divider`, then 26px gap, then a centered flex row of contact items (icon + label) — email, GitHub, LinkedIn — sourced from `siteConfig.contacts`. On desktop the row fits on one line; on narrow viewports it **wraps** (one item per line or two-up depending on width) rather than stacking unconditionally. Inter-item gap: 28px. Icon-to-label gap: 7px. Contact labels use the `caption` token in `--color-text-muted`; icons render at `size={15}` of **visible ink** via the cropped wrappers in `components/icons.tsx` (see §5.x Footer component note). The footer is identical across every page. A **minimum 80px gap** separates the main content from the footer (top divider); on short pages the `flex-1` main expands so the footer still sits at the bottom of the viewport, but tall/scrolling pages never let content crowd the footer.
...
- **`<Footer />`** — renders contact icon rows from `siteConfig.contacts` plus the version tag. The `kind` field on each contact is a discriminated union (see §6.1) so the component imports its icon directly from the config entry, with no internal switch statement. Icons render at a uniform `size={15}`. The config references **cropped** icon wrappers from `components/icons.tsx` (not Phosphor directly): Phosphor pads each glyph differently inside its 256-unit box, so its components render visibly unequal and leave invisible space between icon and label — the wrappers inline the path with the viewBox cropped to the path bounds, so `size` maps 1:1 to visible ink.
